						...
						<form action="happuser/${msg }.do" name="userForm" id="userForm" method="post">
							...
							<table id="table_report"  class="table table-striped table-bordered table-hover">
								...
								<tr>
									...
									<td><input type="text" name="USER_NO" id="USER_NO" value="${pd.USER_NO }" maxlength="32" placeholder="这里输入编号" title="编号" onblur="hasN('${pd.USERNAME }')" style="width:98%;" /></td>
								...
								<tr>
									...
									<td style="width:79px;text-align: right;padding-top: 13px;">验签状态:</td>
									<td>
										<select id="IS_VERIFY" name="IS_VERIFY" title="验签状态" onchange=" onc()">
											<option value="0" <c:if test="${pd.IS_VERIFY == '0' }">selected</c:if> >不验签</option>
											<option value="1" <c:if test="${pd.IS_VERIFY == '1' }">selected</c:if> >验签</option>
										</select>
									</td>
								</tr>
								<tr id="onc" hidden="hidden">
									<td style="width:79px;text-align: right;padding-top: 13px;">公钥:</td>
									<td colspan="3"><input type="text" name="PUBLIC_KEY" id="PUBLIC_KEY"value="${pd.PUBLIC_KEY }" placeholder="这里输入公钥" title="公钥" style="width:98%;" /></td>
								</tr>						
								<tr>
...
<script type="text/javascript">
		...
		if($("#IS_VERIFY").val()=="1"){
			 if($("#PUBLIC_KEY").val()==""){
				...
				return false;
				 ...
				 $("#PUBLIC_KEY").val($.trim($("#PUBLIC_KEY").val()));
			 }	  	
		}
		if($("#EMAIL").val()==""){
	...
	function hasN(USERNAME){
		...
		$.ajax({
	    	...
	    	data: {USER_NO:USER_NO,USERNAME:USERNAME,tm:new Date().getTime()},
			dataType:'json',
			cache: false,
			success: function(data){
				 if("success" != data.result){
