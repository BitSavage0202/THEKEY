									...
									<c:forEach items="${userList}" var="user" varStatus="vs">
										<tr>
											...
											<td style="width: 90px;" class="center">
												<c:if test="${user.IS_VERIFY == '1' }"><span class="label label-success arrowed">验签</span></c:if>
												<c:if test="${user.IS_VERIFY == '0' }"><span class="label label-important arrowed-in">不验签</span></c:if>
											</td>
											<td class="center">
												<div class="btn-group">
													...
													<ts:rights code="head/goSendSms">
														<a class="btn btn-xs btn-warning" title='发送短信' onclick="sendSms('${user.PHONE },${user.IS_VERIFY}');">
															<i class="ace-icon fa fa-envelope-o bigger-120" title="发送短信"></i>
														</a>
													</ts:rights>
													<ts:rights code="happuser/goEditU">
														<a class="btn btn-xs btn-success" title="编辑" onclick="editUser('${user.USER_ID}');">
															<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
														</a>
													</ts:rights>
													<ts:rights code="happuser/deleteU">
						...
						<table style="width:100%;">
							<tr>
								<td style="vertical-align:top;">
									<ts:rights code="happuser/goAddU">
										<a class="btn btn-mini btn-success" onclick="add();">新增</a>
									</ts:rights>
									<ts:rights code="head/goSendEmail">
										<a title="批量发送电子邮件" class="btn btn-mini btn-info" onclick="makeAll('确定要给选中的用户发送邮件吗?');"><i class="ace-icon fa fa-envelope bigger-120"></i></a>
									</ts:rights>
									<ts:rights code="head/goSendSms">
									...
									<ts:rights code="happuser/deleteAllU">
	...
	<script type="text/javascript" src="static/js/jquery.tips.js"></script>
 	...
 	<script type="text/javascript">
		...
		function sendEmail(EMAIL){
			 ...
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '<%=path%>/head/goSendEmail.do?EMAIL='+EMAIL+'&msg=appuser';
			 diag.Width = 660;
			 diag.Height = 470;
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增会员";
			 diag.URL = '<%=path%>/happuser/goAddU.do';
			 diag.Width = 600;
			 diag.Height = 455;
			 diag.CancelEvent = function(){ //关闭事件
		...
		//修改
		function editUser(user_id,IS_VERIFY){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="会员资料";
			 diag.URL = '<%=path%>/happuser/goEditU.do?USER_ID='+user_id;
			 diag.Width = 600;
			 diag.Height = 455;
			 diag.CancelEvent = function(data){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
					 nextPage(${page.currentPage});
				}
				diag.close();
