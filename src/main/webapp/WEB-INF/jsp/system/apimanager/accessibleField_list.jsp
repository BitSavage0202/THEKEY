									...
									<c:forEach items="${accessList}" var="access" varStatus="vs">
										<tr>
											<td class='center' style="width: 30px;">
												<label><input type='checkbox' name='ids' value="${access.DR_ID }" colname="${access.COLUMN_NAME}" class="ace"/><span class="lbl"></span></label>
											</td>
											...
											<td class="center">
												<div class="btn-group">
													<ts:rights code="col/goRule">
														<a class="btn btn-xs btn-purple" title="数据权限管理" onclick="dataJurisdiction('${access.DR_ID}','${access.IN_ID}');">数据权限管理</a>
													</ts:rights>
													...
													<a class="btn btn-xs btn-danger" title="删除" onclick="delAccess('${access.DR_ID }','${access.COLUMN_NAME}');">
														<i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
													</a>
													</ts:rights>
													<ts:rights code="col/goRule">
														<a class="btn btn-xs btn-purple" title="规则管理" onclick="ruleManager('${access.DR_ID}','${access.COLUMN_NAME}');">规则管理</a>
													</ts:rights>
												</div>
											</td>
										</tr>
 	...
 	<script type="text/javascript">
		...
		function editAccess(dr_id){
			 ...
			 diag.show();
				...
				//删除
		function delAccess(drId, columnName){
			var url = "<%=basePath%>api/checkAllDRID.do?DR_IDS="+drId;
			$.get(url,function(data){
				...
				bootbox.confirm(tip, function(result) {
					if(result) {
						top.jzts();
						var url = "<%=basePath%>api/deleteA.do?DR_ID="+drId+"&columnName="+columnName+"&tm="+new Date().getTime();
						$.get(url,function(data){
							if("success" == data.result){
		...
		//设置数据权限
		function dataJurisdiction(dr_id,in_id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="数据权限";
			 diag.URL = '<%=path%>/col/goRule.do?DR_ID='+dr_id+'&IN_ID='+in_id;
			 diag.Width = 800;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				nextPage(${page.currentPage});
		...
		function makeAll(msg){
				...
				var url = "<%=basePath%>api/checkAllDRID.do?DR_IDS="+str;
				$.get(url,function(data){
					var tip = "";
					if("success" == data.result){
						tip ="<font color='#EEEE00' size='4'><b>提示：</b></font><b>确定要删除此数据吗?</b>";
							...
							if(str==''){
								...
								if("success" == data.result){
									...
									$.ajax({
								    	...
								    	data: {DR_IDS:str,columnName:colname},
										dataType:'json',
		...
		//设置数据权限
		function ruleManager(dr_id,colName){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="规则管理";
			 diag.URL = '<%=path%>/pay/checkList.do?DR_ID='+dr_id+'&colName='+colName;
			 diag.Width = 800;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				nextPage(${page.currentPage});
