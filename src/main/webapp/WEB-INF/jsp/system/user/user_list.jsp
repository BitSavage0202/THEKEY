						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;"> 
									...
									<option value="1" <c:if test="${pd.STATUS == '1' }">selected</c:if> >冻结</option>
									</select>
								</td>
								
									<td style="vertical-align:top;padding-left:2px;"><a class="btn btn-light btn-xs" onclick="searchs();"  title="检索"><i id="nav-search-icon" class="ace-icon fa fa-search bigger-110 nav-search-icon blue"></i></a></td>
<!--								<td style="vertical-align:top;padding-left:2px;"><a class="btn btn-light btn-xs" onclick="resetForm();"  title="重置"><i id="nav-search-icon" class="ace-icon fa bigger-110 nav-search-icon blue">重置</i></a></td> -->
								...
								<c:when test="${not empty userList}">
									<c:forEach items="${userList}" var="user" varStatus="vs" >
										...
										<tr>
											...
											<td class="center"><ts:rights code="user/view" > <a onclick="viewUser('${user.USER_ID}','${user.USERNAME}')" style="cursor:pointer;">${user.USERNAME }</a><ts:norights>${user.USERNAME }</ts:norights></ts:rights></td>
											<td class="center">${user.NAME }</td>
											<td class="center">${user.PHONE }</td>
											<td class="center"> <ts:rights code="user/goSendEmail" > <a title="发送电子邮件" style="text-decoration:none;cursor:pointer;" onclick="sendEmail('${user.EMAIL }');">${user.EMAIL }&nbsp;<i class="ace-icon fa fa-envelope-o"></i></a><ts:norights> ${user.EMAIL }</ts:norights></ts:rights></td>
											...
											<td class="center">
												<div class="hidden-sm hidden-xs btn-group">
													<ts:rights code="user/roles">
														<a class="btn btn-xs btn-purple" onclick="editRoles('${user.USER_ID }');">
															角色设置
														</a>
													</ts:rights>
...
function delUser(userId,msg){
	bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
		if(result) {
			top.jzts();
			var url = "<%=basePath%>user/delete.do?USER_ID="+userId+"&tm="+new Date().getTime();
...
function editUser(user_id){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=path%>/user/toEdit.do?USER_ID='+user_id;
	 diag.Width = 469;
	 diag.Height = 510;
	 diag.CancelEvent = function(){ //关闭事件
		 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
...
function sendSms(phone){
	 ...
	 diag.Drag=true;
	 diag.Title ="发送短信";
	 diag.URL = '<%=path%>/head/goSendSms.do?PHONE='+phone+'&msg=appuser';
	 diag.Width = 600;
	 diag.Height = 265;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function sendEmail(EMAIL){
	 ...
	 diag.Drag=true;
	 diag.Title ="发送电子邮件";
	 diag.URL = '<%=path%>/head/goSendEmail.do?EMAIL='+EMAIL;
	 diag.Width = 660;
	 diag.Height = 486;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function sendFhsms(username){
	 ...
	 diag.Drag=true;
	 diag.Title ="站内信";
	 diag.URL = '<%=path%>/fhsms/goAdd.do?username='+username;
	 diag.Width = 660;
	 diag.Height = 444;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
...
function viewUser(USER_ID,USERNAME){
	if('admin' == USERNAME){
	 ...
	 diag.Drag=true;
	 diag.Title ="资料";
	 diag.URL = '<%=path%>/user/view.do?USER_ID='+USER_ID+'&USERNAME='+USERNAME;
	 diag.Width = 469;
	 diag.Height = 380;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
	...
	//角色设置
function editRoles(USER_ID){
	 top.jzts();
	 var diag = new top.Dialog();
	 diag.Drag = true;
	 diag.Title = "用户角色设置";
	 diag.URL = '<%=path%>/user/roles.do?USER_ID='+USER_ID;
	 diag.Width = 650;
	 diag.Height = 550;
	 diag.CancelEvent = function(){ //关闭事件
		diag.close();
